...
class RBM:
    ...
    def init_parameter(self):
        ...
        with tf.variable_scope('Network_parameter'):
            # W, bh, bv is a weight, hidden layer bias, and visible layer bias respectively
    ...
    def _sample_h(self, v):
        ...
        with tf.name_scope('sampling_hidden_units'):
            ...
            # V = Size of visible layer
            # W = (V, H) matrix, v = (?, V) matrix
            # The line below returns a (?, H) matrix)
            a = tf.nn.bias_add(tf.matmul(v, self.W), self.bh)
            p_h_v = tf.nn.sigmoid(a) # (?, H) matrix
    ...
    def _sample_v(self, h):
        ...
        with tf.name_scope('sampling_visible_units'):
            # H = Size of hidden layer
            # V = Size of visible layer
            # W = (V, H) matrix, h = (?, H) matrix
            a = tf.nn.bias_add(tf.matmul(h, tf.transpose(self.W, [1,0])), self.bv)  # (?, V)
    ...
    def inference(self, v):
        ...
        @return sampled visible neurons (value 1 or 0 accroding to Bernouille distribution)
        '''
        p_h_v = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(v, self.W), self.bh))        # (1, H)
        h_ = self._bernouille_sampling(p_h_v, shape=[1, int(p_h_v.shape[-1])])      # (1, H)

        p_v_h = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(h_, tf.transpose(self.W, [1,0])), self.bv))  # (1, V)
        v_ = self._bernouille_sampling(p_v_h, shape=[1, int(p_v_h.shape[-1])])                      # (1, V)
    ...
    def _compute_gradients(self, v0, vk, ph0, phk):
        ...
        def body(i, v0, vk, ph0, phk, dW, dbh, dbv):
            ...
            dw_ = tf.subtract(tf.multiply(ph0_, v0_), tf.multiply(phk_, vk_))   # (V, H) matrix
